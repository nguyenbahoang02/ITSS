...
public class GDWorkerXemTongQuanNamController implements Initializable {
    @FXML
    private TableView<WorkerTimeSheetYear> table;
    @FXML
    private TableColumn<WorkerTimeSheetYear, String> workHoursCol;
    @FXML
    private TableColumn<WorkerTimeSheetYear, String> yearCol;
    @FXML
    private TableColumn<WorkerTimeSheetYear, String> overtimeHoursCol;
    @FXML
    ...
    private ObservableList<WorkerTimeSheetYear> WorkerTimeSheetYearsList;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        List<WorkerTimeSheetYear> workerTimeSheetYears = new ArrayList<>();
        for (int year : years){
            workerTimeSheetYears.add(new WorkerTimeSheetYear(year,0,0));
        }
        for(WorkerTimeSheet oneDay : workerTimeSheetList){
            for (WorkerTimeSheetYear oneYear : workerTimeSheetYears){
                Date dateParse= null;
                try {
                ...
                if ((dateParse.getYear() + 1900) == oneYear.getYear()){
                    oneYear.setWorkHours(oneDay.getWorkHoursShift1() + oneDay.getWorkHoursShift2() + oneYear.getWorkHours());
                    oneYear.setOvertimeHours(oneDay.getWorkHoursShift3() + oneYear.getOvertimeHours());
                }
            }
        }
        WorkerTimeSheetYearsList = FXCollections.observableArrayList(
                workerTimeSheetYears
        );
        table.setItems(WorkerTimeSheetYearsList);
        yearCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetYear, String>("year"));
        workHoursCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetYear, String>("workHours"));
        overtimeHoursCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetYear, String>("overtimeHours"));
    ...
    private void animationSidebar(double translateXSidebar, double translateXItem, double tableColumnWidth) {
        ...
        yearCol.setPrefWidth(yearCol.getPrefWidth() + tableColumnWidth);
