...
import com.example.demo2.Model.Worker.WorkerTimeSheetMonth;
...
public class GDWorkerXemTongQuanController implements Initializable {
    @FXML
    private TableView<WorkerTimeSheetMonth> table;
    @FXML
    private TableColumn<WorkerTimeSheetMonth, String> workHoursCol;
    @FXML
    private TableColumn<WorkerTimeSheetMonth, String> monthCol;
    @FXML
    private TableColumn<WorkerTimeSheetMonth, String> overtimeHoursCol;
    @FXML
    ...
    private ObservableList<WorkerTimeSheetMonth> WorkerTimeSheetMonthsList;

    private List<WorkerTimeSheetMonth> createTimeSheetOfYear(int year){
        List<WorkerTimeSheetMonth> WorkerTimeSheetMonths = new ArrayList<>();
        for (int i = 1; i <= 12; i++){
            WorkerTimeSheetMonths.add(new WorkerTimeSheetMonth(year, i,0,0));
        }
        return WorkerTimeSheetMonths;
    }
    ...
    private List<WorkerTimeSheetMonth> createNewTimeSheetChangeYear(int year){
        ...
        List<WorkerTimeSheetMonth> WorkerTimeSheetMonths = createTimeSheetOfYear(choiceBoxYear.getValue());
        for (WorkerTimeSheet oneDay : workerTimeSheetList){
            for (WorkerTimeSheetMonth oneMonth : WorkerTimeSheetMonths){
                Date dateParse= null;
                try {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        List<WorkerTimeSheetMonth> WorkerTimeSheetMonths = createNewTimeSheetChangeYear(choiceBoxYear.getValue());
        Collections.sort(WorkerTimeSheetMonths);
        WorkerTimeSheetMonthsList = FXCollections.observableArrayList(
                WorkerTimeSheetMonths
        );
        table.setItems(WorkerTimeSheetMonthsList);
        monthCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetMonth, String>("month"));
        workHoursCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetMonth, String>("workHours"));
        overtimeHoursCol.setCellValueFactory(new PropertyValueFactory<WorkerTimeSheetMonth, String>("overtimeHours"));
        ...
        choiceBoxYear.setOnAction(event -> {
            List<WorkerTimeSheetMonth> WorkerTimeSheetMonthsNew = createNewTimeSheetChangeYear(choiceBoxYear.getValue());
            Collections.sort(WorkerTimeSheetMonthsNew);
            WorkerTimeSheetMonthsList = FXCollections.observableArrayList(
                    WorkerTimeSheetMonthsNew
            );
            table.setItems(WorkerTimeSheetMonthsList);
            RangeTimeView.setYear(choiceBoxYear.getValue());
        });
